...
public class ElderGuardianListener implements Listener {
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        if (event.getEntity() instanceof ElderGuardian) {
            ...
            try {
                PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT Resistance FROM diff_elderguardian_settings WHERE ID = " + DayListener.getCurrentDay());
