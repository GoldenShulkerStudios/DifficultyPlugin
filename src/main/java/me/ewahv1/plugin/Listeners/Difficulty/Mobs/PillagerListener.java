...
public class PillagerListener implements Listener {
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        if (event.getEntity() instanceof Pillager) {
            ...
            try {
                PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT DoubleDamage, CriticPercentage, ArrowEffectInstantDamage, ArrowTier FROM diff_pillager_settings WHERE ID = " + DayListener.getCurrentDay());
