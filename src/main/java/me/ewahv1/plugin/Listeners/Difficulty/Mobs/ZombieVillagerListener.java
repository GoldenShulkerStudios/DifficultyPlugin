...
public class ZombieVillagerListener implements Listener {
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        if (event.getEntityType() == EntityType.ZOMBIE_VILLAGER) {
            ...
            try {
                PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT AxeMaterial, Sharpness FROM diff_zombievillager_settings WHERE ID = " + DayListener.getCurrentDay());
