...
public class ZombifiedPiglinListener implements Listener {
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        if (event.getEntity() instanceof PigZombie) {
            ...
            try {
                PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT Speed, BlazeRod, FireAspect FROM diff_zombifiedpiglin_settings WHERE ID = " + DayListener.getCurrentDay());
