...
public class EndermiteListener implements Listener {
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        if (event.getEntity() instanceof Endermite) {
            ...
            try {
                PreparedStatement ps = Connection.getConnection().prepareStatement("SELECT Inmortal, Strength FROM endermite_settings WHERE ID = ?");
                ps.setInt(1, endermite.getEntityId());
